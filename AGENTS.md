...
4. **排查问题时要查看周边引用的独立文件**：大文件拆分成多个文件后，相关逻辑可能分散在多个独立文件中（如 InputArea 拆出了 use-input-area-effects.ts、use-input-area-selectors.ts 等），排查时不能只盯着主文件，必须主动查看所有 import/export 引用的周边文件，否则容易遗漏根因
5. C# 用 partial class，TypeScript 用 export/import 模块化
6. 以下情况不需要强行拆分：
