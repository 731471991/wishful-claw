...
**验证标准**：设定目标（如"修复所有 TypeScript 编译错误"）→ Agent 自主拆分为多个计划 → 每个计划自主走探索→规划→执行→验证 → 进度面板实时更新 → 可中断 → 目标达成或用户中止。
...
v2-iter-8（计划模式）是地基 — v2-iter-9（Goal 模式）复用它去掉人工确认、加多计划编排；v2-iter-10（全局编排）最后接上读任务文件。
