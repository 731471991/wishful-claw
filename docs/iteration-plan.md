...
### v2-iter-8：Global 全局会话 + 项目编排工具

**目标**：全局会话作为通用小助手，不绑项目目录，可正常聊天问答；同时提供项目管理工具，支撑从渠道（微信等）查看项目状态、推动项目任务的场景。

**核心场景**：用户通过微信聊天 → 全局 Agent 接收 → 查看某项目状态 → 有活跃会话就不安排，没有就创建会话发任务。全局 Agent 拿到项目路径后可直接用现有文件工具（Read/Glob/Grep）读项目文档，不需要额外造工具。

| 步骤 | 内容 |
|------|------|
| 1 | 全局会话定义 — 不绑定项目目录，通用聊天助手，Agent 有完整的对话和文件工具能力（但不限定工作区范围） |
| 2 | 新建会话入口 — 新建会话时可选"全局模式"或选择项目 |
| 3 | 项目管理工具注册 — 给全局 Agent 注册以下工具 |
| 4 | 前端 UI — 会话列表中区分全局会话和项目会话，全局会话不显示项目标签 |
| 5 | 渠道路由 — 渠道消息（微信等）路由到全局会话，全局 Agent 通过工具编排项目会话 |
...
| `get_active_sessions` | 查某项目下有没有活跃中的会话 |
| `create_session` | 给项目创建新会话 |
| `send_session_message` | 向会话发送任务消息 |

**Agent 决策逻辑**（由 LLM 自主判断）：查项目 → 看有没有活跃会话 → 有活跃会话就等（不安排）、没有就建会话发任务。全局 Agent 拿到项目路径后可直接用现有 Read/Glob/Grep 读项目文档，无需额外工具。

**验证标准**：新建全局会话 → 正常聊天问答 → 微信发消息"xx项目做完没" → Agent 调 list_projects 查到项目 → 调 get_active_sessions 看会话状态 → 有活跃会话就回复"正在执行中"，没有就调 create_session + send_session_message 推动任务 → 全局会话本身不绑项目目录。
...
v2-iter-7（主聊天接入工作台模式）  ← 当前最高优先级，无前置依赖
  ↓
v2-iter-8（Global 全局会话 + 项目编排工具）  ← 依赖会话管理 + 工具注册框架
  ↓
v2-iter-9（Goal 模式接入）  ← 依赖工作台 UI（Agent 自主操作的展示框架）
```

v2-iter-1 ~ v2-iter-6 已全部完成（tag v2.6.0）。
v2-iter-7（主聊天折叠块模式）已完成，tag v2.7.0。当前最高优先级是 v2-iter-8（Global 全局会话 + 项目编排工具）。
