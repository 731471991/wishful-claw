...
| ID | 名称 | 定位 | 来源 |
|---|---|---|---|
| `default` | WishfulClaw | 默认助手，专业简洁 | 从当前 system-prompt.ts 的风格迁移 |
| `lao-zheng` | 老郑 | 技术搭档兄弟，直接果断有洁癖 | 搬 `D:\work\2026-04\lao-zheng` 的文件（适配命名） |
| `jarvis` | 贾维斯 | 英式管家，礼貌优雅，主动汇报 | 新写 |
| `minimalist` | 极简执行者 | 只给结论不废话 | 参考 KodaClaw minimalist-executor 预设 |
| `analyst` | 深度分析师 | 展示推理过程，不急下结论 | 参考 KodaClaw deep-analyst 预设 |
| `mentor` | 编程导师 | 耐心教学，解释原理 | 新写，参考 OpenCowork patient-guide |
