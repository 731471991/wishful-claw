...
export function useFileTreeActions(state: FileTreeState, options: UseFileTreeActionsOptions = {}) {
  ...
  useEffect(() => {
    if (
      ...
      lastAgentCommandIdRef.current === options.agentCommand.id
    )
      return
    lastAgentCommandIdRef.current = options.agentCommand.id

    if (options.agentCommand.type === 'new-file') {
      state.setAgentRootExpanded(true)
      void handleNewFile(state.workingFolder)
      return
    }
    if (options.agentCommand.type === 'new-folder') {
      state.setAgentRootExpanded(true)
      void handleNewFolder(state.workingFolder)
      return
    }
    if (options.agentCommand.type === 'refresh') {
      void state.refreshTree()
      return
    }
    if (options.agentCommand.type === 'collapse-all') {
      state.setAgentRootExpanded(true)
      handleCollapseAll()
    }
  }, [
