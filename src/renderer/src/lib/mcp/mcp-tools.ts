 ...
 *
 * Execution is owned by the .NET Native Worker (AgentRuntimeMcpExecutor);
 * the renderer keeps definitions only.
 */
...
export function parseMcpToolName(
  prefixedName: string
): { server: string; tool: string; toolName: string } | null {
  ...
  return { server: serverId, tool, toolName: tool }
}
