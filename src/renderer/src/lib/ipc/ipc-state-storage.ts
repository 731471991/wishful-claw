...
async function ipcInvoke(channel: string, payload: unknown): Promise<unknown> {
  // Use window.api.invoke which routes through preload's MessagePack IPC bridge
...
export function createIpcStateStorage({
  ...
  return {
    getItem: async (name: string): Promise<string | null> => {
      try {
        const value = await ipcInvoke(getChannel, name)
    ...
    setItem: async (name: string, value: string): Promise<void> => {
      ...
      try {
        await enqueueWrite(name, async () => {
          await ipcInvoke(setChannel, { key: name, value: parsed })
        })
      } catch {
    ...
    removeItem: async (name: string): Promise<void> => {
      ...
      try {
        await enqueueWrite(name, async () => {
          await ipcInvoke(setChannel, { key: name, value: undefined })
        })
      } catch {
        // Silently fail
      }
