...
public static class AgentRuntimeWebFetchExecutor
{
    ...
    {
        var client = WorkerHttpClientFactory.Create(allowAutoRedirect: true, maxAutomaticRedirections: 5);
    ...
    private static List<string> ReadUrls(JsonElement input)
    {
        ...
        if (!input.TryGetProperty("urls", out var rawUrls)) return urls;

        if (rawUrls.ValueKind == JsonValueKind.String && rawUrls.GetString() is { Length: > 0 } url)
        {
            urls.Add(url.Trim());
            return urls;
        }

        if (rawUrls.ValueKind != JsonValueKind.Array) return urls;

        foreach (var item in rawUrls.EnumerateArray())
        {
            if (item.ValueKind == JsonValueKind.String && item.GetString() is { Length: > 0 } itemUrl)
    ...
    private static async Task<WebFetchResult> FetchUrlAsync(string targetUrl, string format, int timeoutMs, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (response.Content.Headers.ContentLength is > MaxResponseBytes)
                throw new InvalidOperationException($"Response exceeds the {MaxResponseBytes} byte WebFetch limit");
        ...
        {
            var message = timeoutCts.IsCancellationRequested ? $"Request timeout after {timeoutMs}ms" : ex.Message;
            return new WebFetchResult(targetUrl, null, null, string.Empty, format, message);
        }
        finally
        {
            if (slotAcquired) FetchSlots.Release();
        }
    }
    ...
    {
        var match = Regex.Match(html, $"<{tagName}\\b[^>]*>([\\s\\S]*?)</{tagName}>", RegexOptions.IgnoreCase);
            ...
            Regex.Replace(
                Regex.Replace(
                    Regex.Replace(
                        ...
                        "<script\\b[^>]*>[\\s\\S]*?</script>", " ", RegexOptions.IgnoreCase),
                    "<style\\b[^>]*>[\\s\\S]*?</style>", " ", RegexOptions.IgnoreCase),
                "<noscript\\b[^>]*>[\\s\\S]*?</noscript>", " ", RegexOptions.IgnoreCase),
            "<(nav|header|footer|aside|form|button|svg|canvas)\\b[^>]*>[\\s\\S]*?</\\1>", " ", RegexOptions.IgnoreCase);
    }

    private static string ExtractTitleFromHtml(string html) => StripHtml(ExtractHtmlTagContent(html, "title"));
    ...
    private static string ConvertHtmlToMarkdown(string html, string baseUrl)
    {
        ...
        markdown = Regex.Replace(markdown, "<pre\\b[^>]*><code\\b[^>]*>([\\s\\S]*?)</code></pre>",
            match => $"\n\n@@CODE_BLOCK_START@@\n{DecodeHtml(match.Groups[1].Value).Trim()}\n@@CODE_BLOCK_END@@\n\n", RegexOptions.IgnoreCase);
        markdown = Regex.Replace(markdown, "<pre\\b[^>]*>([\\s\\S]*?)</pre>",
            match => $"\n\n@@CODE_BLOCK_START@@\n{StripHtml(match.Groups[1].Value)}\n@@CODE_BLOCK_END@@\n\n", RegexOptions.IgnoreCase);
        markdown = Regex.Replace(markdown, "<h([1-6])\\b[^>]*>([\\s\\S]*?)</h\\1>",
        ...
        markdown = Regex.Replace(markdown, "<blockquote\\b[^>]*>([\\s\\S]*?)</blockquote>",
            match => "\n\n" + string.Join('\n', ConvertInlineHtmlToMarkdown(match.Groups[1].Value, baseUrl).Split('\n').Select(static l => l.Trim()).Where(static l => l.Length > 0).Select(static l => $"> {l}")) + "\n\n", RegexOptions.IgnoreCase);
        markdown = Regex.Replace(markdown, "<li\\b[^>]*>([\\s\\S]*?)</li>",
        ...
        markdown = Regex.Replace(markdown, "<(p|div|section|article|main)\\b[^>]*>([\\s\\S]*?)</\\1>",
            match => { var converted = ConvertInlineHtmlToMarkdown(match.Groups[2].Value, baseUrl); return converted.Length > 0 ? $"\n\n{converted}\n\n" : "\n"; }, RegexOptions.IgnoreCase);
        ...
        return NormalizeMarkdownOutput(DecodeHtml(markdown)
    ...
    {
        var value = input;
        value = Regex.Replace(value, "<img\\b[^>]*src=[\"']([^\"']+)[\"'][^>]*alt=[\"']([^\"']*)[\"'][^>]*>",
            match => $"![{StripHtml(match.Groups[2].Value)}]({ResolveAbsoluteUrl(match.Groups[1].Value, baseUrl)})", RegexOptions.IgnoreCase);
        value = Regex.Replace(value, "<img\\b[^>]*alt=[\"']([^\"']*)[\"'][^>]*src=[\"']([^\"']+)[\"'][^>]*>",
            match => $"![{StripHtml(match.Groups[1].Value)}]({ResolveAbsoluteUrl(match.Groups[2].Value, baseUrl)})", RegexOptions.IgnoreCase);
        value = Regex.Replace(value, "<img\\b[^>]*src=[\"']([^\"']+)[\"'][^>]*>",
            match => $"![]({ResolveAbsoluteUrl(match.Groups[1].Value, baseUrl)})", RegexOptions.IgnoreCase);
        value = Regex.Replace(value, "<a\\b[^>]*href=[\"']([^\"']+)[\"'][^>]*>([\\s\\S]*?)</a>",
            match => { var target = ResolveAbsoluteUrl(match.Groups[1].Value, baseUrl); var label = StripHtml(match.Groups[2].Value); return $"[{(label.Length > 0 ? label : target)}]({target})"; }, RegexOptions.IgnoreCase);
        value = Regex.Replace(value, "<(strong|b)\\b[^>]*>([\\s\\S]*?)</\\1>",
            match => $"**{StripHtml(match.Groups[2].Value)}**", RegexOptions.IgnoreCase);
        value = Regex.Replace(value, "<(em|i)\\b[^>]*>([\\s\\S]*?)</\\1>",
            match => $"*{StripHtml(match.Groups[2].Value)}*", RegexOptions.IgnoreCase);
        value = Regex.Replace(value, "<code\\b[^>]*>([\\s\\S]*?)</code>",
            match => $"`{StripHtml(match.Groups[1].Value).Replace("`", "\\`", StringComparison.Ordinal)}`", RegexOptions.IgnoreCase);
    ...
    private static string ResolveAbsoluteUrl(string value, string baseUrl)
    {
        return Uri.TryCreate(baseUrl, UriKind.Absolute, out var baseUri) &&
            Uri.TryCreate(baseUri, value, out var uri) ? uri.ToString() : value;
    }
    ...
    private static string DecodeHtml(string value) => WebUtility.HtmlDecode(value) ?? string.Empty;
    ...
    private static string EncodeResponse(IReadOnlyList<WebFetchResult> results, string format)
    {
        ...
        using (var writer = new Utf8JsonWriter(stream))
        {
            ...
            foreach (var result in results)
            {
                ...
                if (!string.IsNullOrWhiteSpace(result.FinalUrl)) writer.WriteString("finalUrl", result.FinalUrl);
                if (!string.IsNullOrWhiteSpace(result.Title)) writer.WriteString("title", result.Title);
                writer.WriteString("content", result.Content);
                writer.WriteString("format", result.Format);
                if (!string.IsNullOrWhiteSpace(result.Error)) writer.WriteString("error", result.Error);
    ...
    private sealed record WebFetchResult(string Url, string? FinalUrl, string? Title, string Content, string Format, string? Error);
}
