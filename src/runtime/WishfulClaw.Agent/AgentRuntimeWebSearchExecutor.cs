...
internal static partial class AgentRuntimeWebSearchExecutor
{
    ...
    private static WebSearchResponse ParseProviderJsonResponse(string body, string query, string provider,
        string contentProperty, string? fallbackContentProperty = null)
    {
    ...
    private static List<WebSearchResult> ExtractGoogleResults(string html, int maxResults)
    {
        ...
        for (var index = 0; index < headings.Count && results.Count < maxResults; index++)
        {
            ...
            if (title.Length == 0 || url.Length == 0 || url.Contains("/search?", StringComparison.Ordinal)) continue;
            if (!seen.Add($"{title}::{url}")) continue;

            var start = match.Index;
            var nextStart = index + 1 < headings.Count ? headings[index + 1].Index : Math.Min(start + 6000, html.Length);
            var section = html[start..Math.Min(nextStart, Math.Min(start + 6000, html.Length))];
            var snippet = ExtractSnippet(section, new[] {
    ...
    private static List<WebSearchResult> ExtractBingResults(string html, int maxResults)
    {
        ...
        foreach (Match block in blocks)
        {
            ...
            if (title.Length == 0 || url.Length == 0 || !seen.Add($"{title}::{url}")) continue;
            var snippet = ExtractSnippet(section, new[] {
    ...
    private static List<WebSearchResult> ExtractBaiduResults(string html, int maxResults)
    {
        ...
        for (var index = 0; index < headings.Count && results.Count < maxResults; index++)
        {
            ...
            if (title.Length == 0 || url.Length == 0 || !seen.Add($"{title}::{url}")) continue;
            var start = match.Index;
            var nextStart = index + 1 < headings.Count ? headings[index + 1].Index : Math.Min(start + 4000, html.Length);
    ...
    private static string ExtractSnippet(string section, IReadOnlyList<string> patterns, string title, int groupIndex = 1)
    {
        foreach (var pattern in patterns)
        {
            ...
            if (text.Length > 0 && text != title) return text;
        }
    ...
    private static async Task<HttpTextResponse> SendGetAsync(string url, IReadOnlyDictionary<string, string>? headers, int timeoutMs, CancellationToken ct)
    {
    ...
    private static async Task<HttpTextResponse> SendJsonPostAsync(string url, string body, IReadOnlyDictionary<string, string>? headers, int timeoutMs, CancellationToken ct)
    {
        using var timeoutCts = new CancellationTokenSource(timeoutMs);
        using var linkedCts = CancellationTokenSource.CreateLinkedTokenSource(ct, timeoutCts.Token);
    ...
    private static string ResolveSearchResultUrl(string provider, string rawUrl)
    {
        ...
        if (provider == "google")
        {
            try
            {
                var absolute = normalized.StartsWith("/url?", StringComparison.Ordinal) ? $"https://www.google.com{normalized}" : normalized;
    ...
    private static string? ReadQueryValue(string query, string name)
    {
        ...
        foreach (var part in trimmed.Split('&', StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            if (!string.Equals(Uri.UnescapeDataString(rawName), name, StringComparison.Ordinal)) continue;
    ...
    private static string EncodeResponse(WebSearchResponse response)
    {
        return EncodeJsonObject(writer =>
        {
            ...
            foreach (var result in response.Results)
            {
                ...
                if (result.Score.HasValue) writer.WriteNumber("score", result.Score.Value);
                if (!string.IsNullOrWhiteSpace(result.PublishedDate)) writer.WriteString("publishedDate", result.PublishedDate);
    ...
    private static string EncodeError(string message) => EncodeJsonObject(writer => writer.WriteString("error", message));
    ...
    private sealed record WebSearchConfig(bool Enabled, string Provider, string? ApiKey, int? MaxResults, int? TimeoutMs);
    private sealed record WebSearchRequest(string Query, string Provider, int MaxResults, string SearchMode, string? ApiKey, int TimeoutMs);
    private sealed record WebSearchResponse(List<WebSearchResult> Results, string Query, string Provider, int TotalResults);
    private sealed record WebSearchResult(string Title, string Url, string Content, double? Score, string? PublishedDate);
